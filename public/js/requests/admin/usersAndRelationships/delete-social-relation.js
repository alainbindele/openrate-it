...
jQuery(document).on('ready', function() {
	jQuery('form#deleteSocialRelationForm').bind('submit', function(event){
		...
		function a(form){
			for (var i = 0; i < form.length; i++) {
				if(form[i].checked)
					contacts.push(form[i].value);
			}return contacts;
		};
		jsonRequest.contacts	=	a(form);
		jsonRequest.startUser	=	form.startUid.value;
        ...
        $.ajax({
        	...
        	$("#alert")
            .append("Request:<br><pre>"+JSON.stringify(jsonRequest,undefined,2)+"</pre>")
            .append("Response:<br><pre>"+JSON.stringify(text,undefined,2)+"</pre>").show();
